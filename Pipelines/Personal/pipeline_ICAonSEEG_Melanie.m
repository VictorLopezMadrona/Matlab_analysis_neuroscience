...
cfg=[];
cfg.dataset = cnfg.datasetSEEG_on;
cfg.trialdef.eventvalue = cnfg.eventvalueSEEG_on;
cfg.trialdef.eventtype = cnfg.eventtypeSEEG_on; 
cfg.trialdef.prestim    = cnfg.prestim; %in seconds
cfg.trialdef.poststim   = cnfg.poststim; 
cfg = ft_definetrial(cfg);
ftdataSEEG_on = ft_redefinetrial(cfg,ftdataSEEG_cont);
...
cfg.trialdef.poststim   = cnfg.poststim; 
cfg = ft_definetrial(cfg);
ftdataSEEG_ob = ft_redefinetrial(cfg,ftdataSEEG_cont);

% MERGE conditions
% Check sampling frequency
if ftdataSEEG_on.fsample ~= ftdataSEEG_ob.fsample
    cfg.resamplefs = ftdataSEEG_ob.fsample;
    % I change the Fs of pure tones
    ftdataSEEG_on = ft_resampledata(cfg, ftdataSEEG_on);
end
% Check labels are the same
if length(ftdataSEEG_on.label)~=length(ftdataSEEG_ob.label)
    error('There are not the same channels in both files')
end
m=strcmp(ftdataSEEG_on.label,ftdataSEEG_ob.label);
if mean(m)<1 %there is one or more zeros
    error('Channels in both files are not the same, or not in the same order')
end
%Merge both datasets
cfg = [];
ftdataSEEG =  ft_appenddata(cfg, ftdataSEEG_on, ftdataSEEG_ob);
ftdataSEEG_cont =  concatenate_fttrials(ftdataSEEG);
